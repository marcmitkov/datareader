...
struct OrderDetails {
    ...
    int status;  //     ACCEPTED = 8,    PENDING_PARTIALLY_FILLED = 16,  PARTIALLY_FILLED = 32,
    double payup;
...
public:
    ...
    virtual std::map<int, SymbolInfo> getSymbolTable()=0;
    virtual std::map<OrderId, OrderDetails> getOrderMap()=0;
    virtual void registerCallback(void(*cb)(int, void*), void* clientd, uint64_t delay, bool reoccuring)=0;
#endif
};
