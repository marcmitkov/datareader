...
FuturesMap={
    'ES': 'ESM1', # position file pnl is in points,multiply by 50
    'NQ': 'NQM1',
    'GC':  'GCJ1',
    'CL': 'CLJ1',
    'ZB':  'ZBM1',
...
def GetQuarterlyContractEx(instr,d):
...
def test_automat(date1, root, resdir,datedir,instrumentNoSuffix,ftbase,ftFilter,ftTickFilter,rollFile,debugValue,outputDir,candles,paramdDirIdx,frequencies,smpEnabled,rmLiquidity):
    ...
    if isFutures(instrumentNoSuffix):
        modelFile = 'modelFT_' + frequencies + '.xml'
        #modelFile = 'modelFT.xml'

    else:
        modelFile = 'modelFX_' + frequencies + '.xml'
        #modelFile = 'modelFX.xml'
    ...
    # bttemplate=root + '/backtest-chanel_01_ZOnly.properties',

    auto = velidb.automat.Automat(modbin=root + '/chanelv0_debug', dest=resdir,
                                  modeltemplate= root + '/'+modelFile,
                                  modelparams=modelparams,
                                  bttemplate=root + '/backtest-chanel_01_Prod.properties',
...
def run_automat(date1, root, resdir, datedir, instrumentNoSuffix,ftbase,ftFilter,ftTickFilter,rollFile,debugValue,outputDir,candles,paramdDirIdx,frequencies,smpEnabled,rmLiquidity):
...
if __name__ == '__main__':
    ...
    for instr in instruments:
        ...
        for d in dateInputList:
            print("Running {} date {} with paramIdx {}".format(instr,d,paramdDirIdx))
            if args.test:
                #print('Test Run')
                test_automat(d, root, resdir, d, instr, args.ftbase, args.ftFilter, args.ftTickFilter,
                             args.rollFile, args.chanelDebug, args.outputDir, candles,paramdDirIdx,args.frequency,args.smpEnabled,args.rmLiquidity)
            else:
                p = Process(target=run_automat, args=(
                d, root, resdir, d, instr, args.ftbase, args.ftFilter, args.ftTickFilter, args.rollFile,
                args.chanelDebug, args.outputDir, candles,paramdDirIdx,args.frequency,args.smpEnabled,args.rmLiquidity))
                jobs.append(p)
                p.start()
